...
demo_train <- rename(demo_train,  "id" = "Child.ID")
lu_train <- rename(lu_train,  "id" = "SUBJ")
token_train <- rename(token_train,  "id" = "SUBJ")
...
lu_train$id <- str_replace_all(lu_train$id, "\\.$", "")
token_train$id <- str_replace_all(token_train$id, "\\.$", "")
demo_train$id <- str_replace_all(demo_train$id, "\\.$", "")
...
demo_train_sub <- select(demo_train,id,visit,Diagnosis,Ethnicity,Gender,Age,ADOS,MullenRaw,ExpressiveLangRaw,Socialization)
lu_train_sub <- select(lu_train,id,visit,MOT_MLU,CHI_MLU)
token_train_sub <- select(token_train,id,visit,types_MOT,types_CHI,tokens_MOT,tokens_CHI)
...
train_all <- merge(demo_train_sub, lu_train_sub, all = TRUE)
train_all <- merge(train_all, token_train_sub, all = TRUE)
...
```{r}
visit1 <- select(train_all[train_all$visit==1,],id,visit,ADOS,MullenRaw,ExpressiveLangRaw,Socialization)
visit1 <- select(visit1, id,ADOS,MullenRaw,ExpressiveLangRaw,Socialization)

visit1 <- rename(visit1,  "ADOS1" = "ADOS")
visit1 <- rename(visit1,  "MullenRaw1" = "MullenRaw")
visit1 <- rename(visit1,  "ExpressiveLangRaw1" = "ExpressiveLangRaw")
visit1 <- rename(visit1,  "Socialization1" = "Socialization")
train_all <- merge(train_all, visit1, all = TRUE)
...
```{r}
ids <- data.frame(id=c(unique(train_all$id)))
ids$no <- 1:nrow(ids)
train_all$id <- factor(train_all$id,levels = ids$id,labels = ids$no)

train_all$Gender <- ifelse(train_all$Gender==1,"M","F")
train_all$Diagnosis <-  ifelse(train_all$Diagnosis=="B","TD","ASD")



write_csv(train_all, "assignment-1 results.csv")
```
...
```{r}
unique(filter(train_all, train_all$CHI_MLU>2.7)$id)

unique(filter(train_all, train_all$CHI_MLU<1.5, visit==1)$id)

train_all_na <- train_all
train_all_na = train_all_na %>%
    group_by(id) %>% 
    summarise(id, visit6=(visit==6)) %>%
    inner_join(train_all_na, by ="id") %>%
    filter(visit6==TRUE)
```
...
```{r}
visit6MLU <- filter(train_all, visit==6 & CHI_MLU!="NA")
arrange(visit6MLU, desc(CHI_MLU))$id

visit1MLU <- filter(train_all, visit==1 & CHI_MLU!="NA")
arrange(visit1MLU, CHI_MLU)$id
```
...
```{r}
train_all_d <- filter(train_all, Diagnosis=="ASD", CHI_MLU!="NA", tokens_CHI!="NA")
select(train_all_d, id, id, id)
...
```{r}
train_all_sum <- train_all
train_all_sum = train_all_sum %>% 
    group_by(id) %>% 
    summarise(mean_MLU=mean(CHI_MLU, na.rm=TRUE)) %>%
    inner_join(train_all_sum, by='id')
```
